...
const ProductPage = ({ actions, state }) => {
  ...
  const handleAddToCart = async () => {
    ...
    // if (success && typeof window.fbq === 'function') {
    //   const price = parseFloat(variantForCart?.price || product.price || 0);
    //   window.fbq('track', 'AddToCart', {
    //     content_ids: [variantForCart?.id.toString() || product.id.toString()],
