...
const FriendsPage = () => {
        ...
        await Promise.all(
          data.map(async (friend) => {
            try {
              ...
              wishlistCounts[friend.id] = { count: 0, products: [] };
            }
          })
        );
  ...
  useEffect(() => {
    const loadCounts = async () => {
        ...
        if (user.account_visibility === "public") {
          try {
            ...
            if (res.ok) {
              ...
              expandedStates[user.id] = true; // Set expanded by default
              publicWishlistData[user.id] = data; // Load the wishlist data
            } else {
              counts[user.id] = 0;
      ...
      setExpandedPublic(expandedStates); // Set all public users as expanded
      setPublicWishlists(publicWishlistData); // Set the wishlist data
    };
    ...
    const init = async () => {
      ...
      await Promise.all(
        filtered.map(async (user) => {
          wishlistCounts[user.id] = await fetchUserWishlistCount(user.id);
        })
      );
      ...
      setFriendWishlists((prev) => ({
        ...prev,
        ...Object.fromEntries(
          Object.entries(wishlistCounts).map(([id, count]) => [
            id,
            { ...(prev[id] || {}), count },
          ])
        ),
