...
export const addToCart = async (state, product, selectedVariant = null, quantity = 1) => {
  ...
  if (!isValidJwt) {
    ...
    if (existingItemIndex >= 0) {
      ...
      guestCart.push({
        ...
        name: product.name,
        price: product.price_html || product.price, // adjust based on your data
        quantity: 1,
  ...
  const headers = {
    ...
    ...(isValidJwt && { Authorization: `Bearer ${token}` }), // add only if logged in
  };

  try {
    // Get cart (guest or logged-in)
    const cartRes = await fetch(
      `${getWpBaseUrl(state)}/wp-json/theme/v1/cart`,
      { method: "GET", headers }
    );
    ...
    // Add to cart
    const res = await fetch(
      `${getWpBaseUrl(state)}/wp-json/theme/v1/cart/add`,
      {
        method: "POST",
        headers,
        body: JSON.stringify({
    ...
    if (!res.ok) {
      ...
      showToast(
        data.message || "Maximum quantity reached for this product",
        "warn"
      );
      return false;
    }
...
export const toggleWishlist = async (state, productId) => {
  ...
  await syncWithServer(productId, isAdding);
  return updated;
};
...
export const handleLogin = async (state, actions, token, userData = null) => {
  ...
  const backParam = queryParams.get("back");

  showToast("Login successful!", "success");
  if (backParam) {
    actions.router.set("/" + backParam);
  } else {
    actions.router.set("/");
  }
